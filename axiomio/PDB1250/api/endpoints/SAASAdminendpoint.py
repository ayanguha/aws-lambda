...
from ..handlers.SAASAdminhandler import *
...
@ns.route('/usage/updateAll')
class ManageAccountUsageUpdateInfoHandler(Resource):
    def post(self):
        """
        Add Billing Record
        """

        response,code = createAccountUsageRecords(request)
        return response,code

@ns.route('/<string:accountId>/usage')
class ManageAccountUsageInfoHandler(Resource):
    ...
    def get(self,accountId):
        ...
        response,code = getAccountUsageRecords(request,accountId)
        return response,code
