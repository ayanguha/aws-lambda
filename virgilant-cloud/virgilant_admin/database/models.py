...
def __str2datetime__(s):
    ...
    try:
        ...
        pass
    try:
        return datetime.strptime(s + 'T00:00:00.000Z','%d %B, %YT%H:%M:%S.%fZ')
    except:
        raise


def __date2str__(d):
    #return datetime.strftime(d,'%Y-%m-%d').decode('utf-8', 'ignore')
    return datetime.strftime(d,'%d %B, %Y').decode('utf-8', 'ignore')
...
class LegalInfo(db.Model):
    ...
    def serialize(self):
       return {'id': self.id,
               ...
               'ReleasedDate': __date2str__(self.ReleasedDate),
